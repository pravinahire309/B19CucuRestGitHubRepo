...
public class GETRequestTestSD {
	...
	public void user_checks_the_count_or_size(String jpath, String jpathvalue) throws Throwable {
	    String actualLiming = res.body().jsonPath().getString(jpath);
	    Assert.assertTrue(actualLiming.equals(jpathvalue));
	}*/
	
	@Then("^user checks data at path \"([^\"]*)\" having value \"([^\"]*)\" in jason$")
	public void user_checks_data_at_path_having_value_in_jason(String jsnpath, String pathvalue) throws Throwable {   
		String actualLiming = res.body().jsonPath().getString(jsnpath);
		Assert.assertTrue(actualLiming.equals(pathvalue));
